...
4. Pi keeps snapshots in memory. Its `input` hook stages the current IDs; `before_agent_start` injects them as a hidden persistent custom context message without rewriting the user's prompt, then consumes them.
